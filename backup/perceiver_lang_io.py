...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
            ...
            no_language=False,
            final_dim=64,
            reconstruction3D_pretraining=False,
            decode_occupnacy=True,
            decode_rgb=True,
    ):
        super().__init__()
        self.depth = depth
        ...
        if self.lang_fusion_type == 'concat':
            ...
            self.lang_preprocess = nn.Linear(lang_emb_dim, self.im_channels * 2) ## removed proprio
        ...
        if self.decode_occupnacy:
            ...
            self.normalize_voxel = torch.nn.Sigmoid()
        if self.decode_rgb:
            self.voxel_decoderRGB = Conv3DBlock( #self.final_dim
                self.im_channels + self.final_dim, 3, kernel_sizes=3, strides=1,
                norm=None, activation=activation,
            )
        ...
        if not self.reconstruction3D_pretraining:
            ...
            self.final = Conv3DBlock(
                self.final_dim if (self.no_perceiver or self.no_skip_connection) else self.im_channels + self.final_dim,
                self.final_dim,
                kernel_sizes=3,
    ...
    def forward(
        ...
        # noise = torch.randn(d0.shape).to(ins.device)
        # if add_noise:
        #     # print('noise added')
        #     d0 = d0 + noise
        ...
        ## change - no proprio data for generated data
        # concat proprio
        if self.low_dim_size > 0:
        ...
        for it in range(self.iterations):
            ...
            # if add_noise:
            #     x = x + torch.normal(0, 1.0, size=(x.shape[1], x.shape[2])).to(device)
        ...
        voxel_reconstructed = None
        # voxel decoder                                    # [B,64,100,100,100] -> [B,1,100,100,100]
        if self.decode_occupnacy:
            voxel_reconstructed = self.voxel_decoder(torch.cat([d0, u0], dim=1))
            voxel_reconstructed = self.normalize_voxel(voxel_reconstructed)

        if self.decode_rgb:
            voxel_reconstructed_rgb = self.voxel_decoderRGB(torch.cat([d0, u0], dim=1))
            voxel_reconstructed_rgb = self.normRGB(voxel_reconstructed_rgb)
            voxel_reconstructed = torch.cat((voxel_reconstructed_rgb, voxel_reconstructed), dim=1)

        trans = None; rot_and_grip_out = None; collision_out = None
        if not self.reconstruction3D_pretraining:
            # ablations
