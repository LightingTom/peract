...
class QFunction(nn.Module):
    ...
    def forward(self, rgb_pcd, tar_rgb_pcd, proprio, pcd, tar_pcd, lang_goal_emb, lang_token_embs,
        ...
        q_trans, \
        q_rot_and_grip, \
        q_ignore_collisions, voxel_reconstructed = self._qnet(
...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        for cname in cameras:
            if not torch.equal(replay_sample['tar_%s_rgb' % cname], replay_sample['%s_rgb' % cname]):
                print(cname + '_rgb')
            if not torch.equal(replay_sample['tar_%s_point_cloud' % cname], replay_sample['%s_point_cloud' % cname]):
                print(cname + '_pcd')
        # print(os.getcwd())
