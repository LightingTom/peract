...
class MVPViTLangAndFcsNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 fc_layers: List[int] = None,
                 activation: str = 'relu'):
        super(MVPViTLangAndFcsNet, self).__init__()
    ...
    def build(self):
        ...
        self.encode_lang = nn.Linear(self._lang_feat_dim, 768)
        self.encode_all_input = nn.Linear(768*5, 768)
    ...
    def forward(self, observations, low_dim_ins, lang_goal_emb):
        ...
        b, c, h, w = rgb_depth.shape
        rgb_depth = rgb_depth.reshape(-1, 3, h, w)
        x = self._vit(rgb_depth)
        x = x.reshape(b, len(observations), -1)
        _, _, d = x.shape
        
        low_dim_latents = low_dim_ins.unsqueeze(-1).repeat(1, 1, d)
        combined = torch.cat([x, low_dim_latents], dim=1)

        lang = self.encode_lang(lang_goal_emb).unsqueeze(1)
        x = torch.cat([x, lang], dim=1)
        x = x.reshape(b, -1)
        x = self.encode_all_input(x)
        return self._fcs(x)
