...
# from amsolver.backend import task as amsolver_task
...
def eval_seed(train_cfg,
    ...
    # TODO: change to iterative update of all eval_cfg.method config
    train_cfg.method.masked_decoding = eval_cfg.method.masked_decoding
    ...
    elif eval_cfg.framework.eval_type == 'missing':
        weight_folders = os.listdir(weightsdir)
        weight_folders = sorted(map(int, weight_folders))

        env_data_csv_file = os.path.join(logdir, 'eval_data_new_task.csv')
    ...
    elif '_to_' in eval_cfg.framework.eval_type:
        weight_folders = os.listdir(weightsdir)
        weight_folders = sorted(map(int, weight_folders))

        env_data_csv_file = os.path.join(logdir, 'eval_data_train_tasks.csv')
        # if os.path.exists(env_data_csv_file):
        #     env_dict = pd.read_csv(env_data_csv_file).to_dict()
        #     evaluated_weights = sorted(map(int, list(env_dict['step'].values())))
            # weight_folders = [w for w in weight_folders if w not in evaluated_weights]

        start_weight, _, end_weight = eval_cfg.framework.eval_type.split('_')
        weight_folders = [w for w in weight_folders if w>=int(start_weight) and w<=int(end_weight)]
        weight_folders = [w for w in weight_folders if w % 10000 == 0]

        print('Evaluating weights: ', weight_folders)
    ...
    # evaluate only the last 2 checkpoint
    elif eval_cfg.framework.eval_type == 'last_2':
        weight_folders = os.listdir(weightsdir)
        weight_folders = sorted(map(int, weight_folders))
        weight_folders = [weight_folders[-2], weight_folders[-1]]
        print("Last weights:", weight_folders)
...
def main(eval_cfg: DictConfig) -> None:
    ...
    logdir = os.path.join(eval_cfg.framework.logdir,
                                ...
                                'seed%d' % start_seed)
    if not os.path.exists(os.path.join(logdir, 'generated_imgs')):
        os.mkdir(os.path.join(logdir, 'generated_imgs'))
    os.environ['img_path'] = os.path.join(logdir, 'generated_imgs')
