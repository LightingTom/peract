...
class ViTBCLangAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        observations = []
        for camera_name in self._camera_name:
            observations.append(    
                replay_sample['%s_rgb' % camera_name]
                # replay_sample['%s_point_cloud' % camera_name]
            )
    ...
    def act(self, step: int, observation: dict,
        ...
        for camera_name in self._camera_name:
            observations.append(    
                observation['%s_rgb' % camera_name][0].to(self._device),
        #       observation['%s_point_cloud' % self._camera_name][0].to(self._device)
            )
